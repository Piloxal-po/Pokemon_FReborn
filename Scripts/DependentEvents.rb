...
def pbGetDependencyDirection(eventName)
  event = pbGetDependency(eventName)
  return $game_player.direction if !event || event.opacity <= 0

  return event.direction
end

def pbDependentEventLocation(eventName, x, y)
  event = pbGetDependency(eventName)
  return unless event
...
def pbDependentEventsOpacity(opacity)
