def pbRandomMove
  loop do
    if false
      ...
      next if move > 384 || (move == :SKETCH) || (move == :STRUGGLE)
    end
    return move if getMoveName(move) != ""
  end
end
...
class NonlegendaryRestriction
  def isValid?(pkmn)
    compat10 = $cache.pkmn[pkmn.species][:Abilities][0]
    compat11 = $cache.pkmn[pkmn.species][:Abilities][1]
    genderbyte = $cache.pkmn[pkmn.species][:GenderRatio]
...
def hasMorePowerfulMove(moves, basemove)
  thisdata = moveData(basemove)
...
def pbRandomPokemonFromRule(rule, trainer)
  ...
  begin
    ...
    loop do
      species = 0
      loop do
        species = rand($cache.pkmn.length) + 1
        cname = getMonName(species) rescue nil
        break if cname
      end
      r = rand(20)
    ...
    loop do
      ...
      if nd5 == nm5 || nature == :LAX || nature == :GENTLE
    ...
    loop do
      ...
      case itemsym
        when :LIGHTBALL then next if (species != :PIKACHU)
        when :SHEDSHELL then next if (species != :FORRETRESS) || (species != :SKARMORY)
        when :SOULDEW then next if (species != :LATIOS) || (species != :LATIAS)
        when :LIECHIBERRY && (ev & 0x02) == 0
          if rand(2) == 0
            next
          else
            ev |= 0x02
          end
        when :FOCUSSASH then next if baseStatTotal(species) > 450 && rand(10) < 8
        when :ADAMANTORB then next if (species != :DIALGA)
        when :PASSHOBERRY then next if (species != :STEELIX)
        when :BABIRIBERRY then next if (species != :TYRANITAR)
        when :HABANBERRY then next if (species != :GARCHOMP)
        when :OCCABERRY then next if (species != :METAGROSS)
        when :CHOPLEBERRY then next if (species != :UMBREON)
        when :YACHEBERRY then next if (species != :TORTERRA) && (species != :GLISCOR) && (species != :DRAGONAIR)
        when :SHUCABERRY then next if (species != :HEATRAN)
        when :SALACBERRY && (ev & 0x08) == 0
          if rand(2) == 0
            next
          else
            ev |= 0x08
          end
        when :PETAYABERRY && (ev & 0x10) == 0
          if rand(2) == 0
            next
          else
            ev |= 0x10
          end
        when :DEEPSEATOOTH then next if (species != :CLAMPERL)
        when :THICKCLUB then next if (species != :CUBONE) && (species != :MAROWAK)
      end
      break
    end
    if level < 10
      item = (:ORANBERRY || item) if rand(40) == 0 || (item == :SITRUSBERRY)
    end
    if level > 20
      item = (:SITRUSBERRY || item) if rand(40) == 0 || (item == :ORANBERRY)
    end
    ...
    if (item == :LIGHTCLAY) && !moves.include?((:LIGHTSCREEN || -1)) && !moves.include?((:REFLECT || -1))
      item = :LEFTOVERS
    end
    if (item == :BLACKSLUDGE)
      if $cache.pkmn[species, 0].Type1 != :POISON && $cache.pkmn[species, 0].Type2 != :POISON
        item = :LEFTOVERS
      end
    end
    if (item == :HEATROCK) && !moves.include?((:SUNNYDAY || -1))
      item = :LEFTOVERS
    end
    if (item == :DAMPROCK) && !moves.include?((:RAINDANCE || -1))
      item = :LEFTOVERS
    end
    if moves.include?((:REST || -1))
      item = :LUMBERRY if rand(3) == 0
      item = :CHESTOBERRY if rand(4) == 0
    end
    pk = BTPokemon.new(species, item, nature, moves[0], moves[1], moves[2], moves[3], ev)
...
def pbDecideWinnerScore(party0, party1, rating)
  ...
  for i in 0...party0.length
    for move in party0[i].moves
      ...
      for j in 0...party1.length
        score += pbDecideWinnerEffectiveness(move.move, types1[j], types2[j], abilities[j], [-16, -8, 0, 4, 12, 20])
      end
    end
...
def pbTrainerInfo(pokemonlist, trfile, rules)
  ...
  if bttrainers.length == 0
    for i in 0...200
      ...
      loop do
        ...
        trainerid = :YOUNGSTER if rand(30) == 0
        next if PBTrainers.getName(trainerid) == ""

        money = (!trainertypes[trainerid] ||
               !trainertypes[trainerid].checkFlag?(:moneymult)) ? 30 : trainertypes[trainerid].checkFlag?(:moneymult)
        next if money >= 100
      ...
      tr = [trainerid, randomName, _INTL("Here I come!"), _INTL("Yes, I won!"), _INTL("Man, I lost!"), []]
      bttrainers.push(tr)
    end
    bttrainers.sort! { |a, b|
      money1 = (!trainertypes[a[0]] || !trainertypes[a[0]].checkFlag?(:moneymult)) ? 30 : trainertypes[a[0]].checkFlag?(:moneymult)
      money2 = (!trainertypes[b[0]] || !trainertypes[b[0]].checkFlag?(:moneymult)) ? 30 : trainertypes[b[0]].checkFlag?(:moneymult)
  ...
  for pkmn in pokemonlist
    ...
    pkmntypes.push(getTypes(pkmn.species, pkmn.form))
  ...
  for btt in 0...bttrainers.length
    ...
    newbttrainers.push([trainerdata[0], trainerdata[1], trainerdata[2], trainerdata[3], trainerdata[4], numbers])
  end
  yield(nil) if block_given?
  pbpokemonlist = []
  for pkmn in pokemonlist
    pbpokemonlist.push(BTPokemon.fromPokemon(pkmn))
  end
