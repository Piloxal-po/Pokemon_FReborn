...
def Kernel.pbRockClimb
  ...
  if Kernel.ignoresBadgeRequirements ||
...
def Kernel.pbStrength
  ...
  elsif Kernel.ignoresBadgeRequirements ||
...
def Kernel.pbSurfacing
  ...
  movefinder = Kernel.pbCheckMove(:DIVE)
  if !facingEvent
    if Kernel.ignoresBadgeRequirements || ((movefinder || ($PokemonBag.pbHasItem?(:GOLDENSCUBAGEAR) && $PokemonBag.pbHasItem?(:HM06))) &&
      (HIDDENMOVESCOUNTBADGES ? $Trainer.numbadges >= BADGEFORDIVE : $Trainer.badges[BADGEFORDIVE]))
...
HiddenMoveHandlers::CanUseMove.add(:FLY, lambda { |move, pkmn|
  if !Kernel.ignoresBadgeRequirements && !(HIDDENMOVESCOUNTBADGES ? $Trainer.numbadges >= BADGEFORFLY : $Trainer.badges[BADGEFORFLY])
    Kernel.pbMessage(_INTL("Sorry, a new Badge is required."))
    return false
  end
  if Rejuv
...
HiddenMoveHandlers::CanUseMove.add(:FLASH, lambda { |move, pkmn|
  if !Kernel.ignoresBadgeRequirements && !(HIDDENMOVESCOUNTBADGES ? $Trainer.numbadges >= BADGEFORFLASH : $Trainer.badges[BADGEFORFLASH])
    Kernel.pbMessage(_INTL("Sorry, a new Badge is required."))
    return false
  end
...
def Kernel.ignoresBadgeRequirements
  return $DEBUG && GAMEVERSION == 'dev'
end
