def pbBaseStatTotal(species)
  return $cache.pkmn[species, 0].BaseStats.sum
end
...
def pbTooTall?(pokemon, maxHeightInMeters)
  return pokemon.height > maxHeightInMeters
end

def pbTooHeavy?(pokemon, maxWeightInKg)
  return pokemon.weight > maxWeightInKg
end
...
class EnemyLevelAdjustment < LevelAdjustment
  def initialize(level)
    ...
    @level = [[1, level].max, MAXIMUMLEVEL].min
  end
...
class TotalLevelAdjustment < LevelAdjustment
  def initialize(minLevel, maxLevel, totalLevel)
    super(LevelAdjustment::EnemyTeam)
    @minLevel = [[1, minLevel].max, MAXIMUMLEVEL].min
    @maxLevel = [[1, maxLevel].max, MAXIMUMLEVEL].min
...
class SoulDewClause
  def isValid?(pokemon)
    return !(pokemon.item == :SOULDEW)
  end
end

class ItemsDisallowedClause
  def isValid?(pokemon)
    return !pokemon.hasAnItem?
  end
end

class NegativeExtendedGameClause
  def isValid?(pokemon)
    return false if (pokemon.species == :ARCEUS)
    return false if (pokemon.item == :MICLEBERRY)
    return false if (pokemon.item == :CUSTAPBERRY)
    return false if (pokemon.item == :JABOCABERRY)
    return false if (pokemon.item == :ROWAPBERRY)
  end
end
...
class UnevolvedFormRestriction
  def isValid?(pokemon)
    ...
    canEvolve = ($canEvolve[pokemon.species] != nil) ? $canEvolve[pokemon.species] :
       ($canEvolve[pokemon.species] = (pbGetEvolvedFormData(pokemon.species, pokemon).length != 0))
...
class LittleCupRestriction
  def isValid?(pokemon)
    return false if (pokemon.item == :BERRYJUICE)
    return false if (pokemon.item == :DEEPSEATOOTH)
    return false if pokemon.knowsMove?(:SONICBOOM)
    return false if pokemon.knowsMove?(:DRAGONRAGE)
    return false if (pokemon.species == :SCYTHER)
    return false if (pokemon.species == :SNEASEL)
    return false if (pokemon.species == :MEDITITE)
    return false if (pokemon.species == :YANMA)
    return false if (pokemon.species == :TANGELA)
    return false if (pokemon.species == :MURKROW)
...
module NicknameChecker
  ...
  def check(name, species)
    ...
    for i in @@namesMaxValue..$cache.pkmn.length
      if i != species
        n = getName(i)
...
def pbPrimeCupRules(double)
  ret = PokemonChallengeRules.new
  ret.setLevelAdjustment(OpenLevelAdjustment.new(MAXIMUMLEVEL))
  ret.addTeamRule(SpeciesClause.new)
  ret.addTeamRule(ItemClause.new)
  ret.addBattleRule(SleepClause.new)
  ret.addBattleRule(FreezeClause.new)
