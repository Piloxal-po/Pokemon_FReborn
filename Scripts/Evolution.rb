...
class PokemonEvolutionScene
  ...
  def pbStartScreen(pokemon, newspecies, item = nil)
    ...
    @newform = newspecies[:form]
    @newform = getEvolutionForm(@pokemon, item) if !@newform
  ...
  def pbEvolution(cancancel = true, item = nil)
    ...
    if @canceled
      ...
      if !createSpecies.nil? && $Trainer.party.length < 6
        ...
        newpokemon.species = createSpecies[:species]
        newpokemon.name = getMonName(newpokemon.species, @pokemon.form)
        ...
        $Trainer.pokedex.dexList[newpokemon.species][:seen?] = true
        $Trainer.pokedex.dexList[newpokemon.species][:owned?] = true
...
def getEvolutionForm(mon, item = nil)
  ...
  case species
    ...
    when :PIKACHU then return !Rejuv && Raichu.include?($game_map.map_id) ? 1 : 0
    ...
    when :BERGMITE then return Avalugg.include?($game_map.map_id) ? 1 : 0
    ...
    when :ESPURR, :BASCULIN
      return 1 if mon.isFemale?
      return 0 if mon.isMale?
    else return form
  end
end
...
def checkEvolutionEx(pokemon)
  ...
  if !d.nil?
    for form in d
      ret = yield pokemon, form[:method], form[:parameter], {species: form[:species], form: form[:form]}
      break if !ret.nil?
    end
  else
    ret = nil
...
def pbGetEvolvedFormData(species, pokemon = nil)
  ...
  if pokemon != nil
    return $cache.pkmn[species, pokemon.form].evolutions
  end

  return $cache.pkmn[species, $cache.pkmn[species].baseForm].evolutions
end
