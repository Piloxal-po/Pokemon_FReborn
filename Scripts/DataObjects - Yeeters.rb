...
def monDump(monhash = $cache.pkmn)
  exporttext = "MONHASH = {\n"
  for i in monhash.keys
    ...
    if mon.forms.empty?
      ...
      for form in 0...mon.forms.length
        ...
        if mon.formData.dig(mon.forms[form], :MegaEvolutions)
          exporttext += "    :MegaEvolutions => {\n"
          mon.formData.dig(mon.forms[form], :MegaEvolutions).each { |method, form|
      ...
      exporttext += "  :UltraForm => #{mon.formData.dig(:UltraForm)},\n" if mon.formData.dig(:UltraForm)
    end
    exporttext += "},\n\n"
