...
def pbChooseSpecies(default)
  ...
  $cache.pkmn.each_with_index { |(key, value), i|
    commands.push(_ISPRINTF("{1:03d} {2:s}", i+1, getMonName(key)))
  }
...
module StringProperty
  def self.set(settingname, oldsetting)
    message = Kernel.pbMessageFreeText(_INTL("Set the value for {1}.", settingname), oldsetting ? oldsetting : "", false, 256, Graphics.width)
  end
...
class LimitStringProperty
  ...
  def set(settingname, oldsetting)
    message = Kernel.pbMessageFreeText(_INTL("Set the value for {1}.", settingname), oldsetting ? oldsetting : "", false, @limit)
  end
  ...
  loop do
    ...
    for i in 0...enctype.length
      ...
      if enctype[i][1] == enctype[i][2]
        commands.push(
          _INTL("{1}% {2} (Lv.{3})", ch, getMonName(enctype[i][0]), enctype[i][1])
        )
      else
        commands.push(
          _INTL("{1}% {2} (Lv.{3}-Lv.{4})", ch, getMonName(enctype[i][0]), enctype[i][1], enctype[i][2])
        )
      end
    end
...
module MapCoordsFacingProperty
  def self.set(settingname, oldsetting)
    ...
    if chosenmap >= 0
      mappoint = chooseMapPoint(chosenmap)
      if mappoint
        facing = Kernel.pbMessage(_INTL("Choose the direction to face in."), [_INTL("Down"), _INTL("Left"), _INTL("Right"), _INTL("Up")], -1)
        if facing < 0
          return oldsetting
        else
