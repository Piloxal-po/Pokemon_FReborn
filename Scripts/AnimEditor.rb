...
class Slider < UIControl
  ...
  def refresh
    ...
    x += size
    shadowtext(
      bitmap, x, y, leftarrows.width, height, _INTL(" << "),
    ...
    x += numbers
    shadowtext(
      bitmap, x, y, rightarrows.width, height, _INTL(" >> "),
...
class ControlWindow < SpriteWindow_Base
  ...
  def addControl(control, y: @controls.length)
    i = @controls.length
    @controls[i] = control
    @controls[i].x = 0
    @controls[i].y = y * 32
  ...
  def freeAddControl(control, x: 0, y: 0)
    i = @controls.length
    y = y * 32
    @controls[i] = control
    @controls[i].x = x
  ...
  def addLabel(label, setproc = nil, y: @controls.length)
    return addControl(Label.new(label, setproc), y: y)
  end

  def addButton(label, setproc = nil, y: @controls.length)
    return addControl(Button.new(label, setproc), y: y)
  end

  def addSlider(label, minvalue, maxvalue, curvalue, setproc = nil, y: @controls.length)
    return addControl(Slider.new(label, minvalue, maxvalue, curvalue, setproc), y: y)
  end

  def addOptionalSlider(label, minvalue, maxvalue, curvalue, setproc = nil, y: @controls.length)
    return addControl(OptionalSlider.new(label, minvalue, maxvalue, curvalue, setproc), y: y)
  end

  def addTextSlider(label, options, curvalue, setproc = nil, y: @controls.length)
    return addControl(TextSlider.new(label, options, curvalue, setproc), y: y)
  end

  def addOptionalTextSlider(label, options, curvalue, setproc = nil, y: @controls.length)
    return addControl(OptionalTextSlider.new(label, options, curvalue, setproc), y: y)
  end

  def addCheckbox(label, setproc = nil, y: @controls.length)
    return addControl(Checkbox.new(label, setproc), y: y)
  end

  def addSpace(y = @controls.length)
    return addControl(UIControl.new(""), y: y)
  end
...
class AnimationWindow < SpriteWrapper
  ...
  def refresh
    ...
    @contents.blt(arrowwidth + NUMFRAMES * 96, 0, @arrows.bitmap, Rect.new(arrowwidth, 0, arrowwidth, 96))
    ...
    if @animation.graphic == ""
      ...
      begin
        @animbitmap = AnimatedBitmap.new(
          "Graphics/Animations/" + @animation.graphic,
          @animation.hue
        ).deanimate
      rescue
        @animbitmap = nil
        ...
        if Input.trigger?(Input::C)
          case cmdwin.index
            when 0 # Copy
              ...
              params.setRange(-2, 1000)
              params.setInitialValue(cel[AnimFrame::PATTERN])
              params.setCancelValue(cel[AnimFrame::PATTERN])
              cel[AnimFrame::PATTERN] = Kernel.pbMessageChooseNumber('Pattern Value:', params)
              sliderwin2.controls[set0].curvalue = cel[AnimFrame::PATTERN]
              canvas.setSpriteBitmap(previewsprite, cel)
              pbSpriteSetAnimFrame(previewsprite, cel, nil, nil)
              previewsprite.z = previewwin.z + 1
              sliderwin2.update
            ...
            when 1 # Paste
              params = ChooseNumberParams.new
              params.setRange(-64, 512 + 64)
              params.setInitialValue(sliderwin2.controls[set1].curvalue)
              params.setCancelValue(sliderwin2.controls[set2].curvalue)
              cel[AnimFrame::X] = Kernel.pbMessageChooseNumber('X Value:', params)
              sliderwin2.controls[set1].curvalue = cel[AnimFrame::X]
              canvas.setSpriteBitmap(previewsprite, cel)
              pbSpriteSetAnimFrame(previewsprite, cel, nil, nil)
              previewsprite.z = previewwin.z + 1
              sliderwin2.update
            ...
            when 2 # Clear Frame
              params = ChooseNumberParams.new
              params.setRange(-64, 512 + 64)
              params.setInitialValue(sliderwin2.controls[set2].curvalue)
              params.setCancelValue(sliderwin2.controls[set2].curvalue)
              cel[AnimFrame::Y] = Kernel.pbMessageChooseNumber('Y Value:', params)
              sliderwin2.controls[set2].curvalue = cel[AnimFrame::Y]
              canvas.setSpriteBitmap(previewsprite, cel)
              pbSpriteSetAnimFrame(previewsprite, cel, nil, nil)
              previewsprite.z = previewwin.z + 1
              sliderwin2.update
            ...
            when 3 # Insert Frame
              params = ChooseNumberParams.new
              params.setRange(5, 1000)
              params.setInitialValue(cel[AnimFrame::ZOOMX])
              params.setCancelValue(cel[AnimFrame::ZOOMX])
              cel[AnimFrame::ZOOMX] = Kernel.pbMessageChooseNumber('Zoom X Value:', params)
              sliderwin2.controls[set3].curvalue = cel[AnimFrame::ZOOMX]
              canvas.setSpriteBitmap(previewsprite, cel)
              pbSpriteSetAnimFrame(previewsprite, cel, nil, nil)
              previewsprite.z = previewwin.z + 1
              sliderwin2.update
            ...
            when 4 # Delete Frame
              params = ChooseNumberParams.new
              params.setRange(5, 1000)
              params.setInitialValue(cel[AnimFrame::ZOOMY])
              params.setCancelValue(cel[AnimFrame::ZOOMY])
              cel[AnimFrame::ZOOMY] = Kernel.pbMessageChooseNumber('Zoom Y Value:', params)
              sliderwin2.controls[set4].curvalue = cel[AnimFrame::ZOOMY]
              canvas.setSpriteBitmap(previewsprite, cel)
              pbSpriteSetAnimFrame(previewsprite, cel, nil, nil)
              previewsprite.z = previewwin.z + 1
              sliderwin2.update
            ...
            when 5 # Delete Frame
              params = ChooseNumberParams.new
              params.setRange(0, 359)
              params.setInitialValue(cel[AnimFrame::ANGLE])
              params.setCancelValue(cel[AnimFrame::ANGLE])
              cel[AnimFrame::ANGLE] = Kernel.pbMessageChooseNumber('Angle Value:', params)
              sliderwin2.controls[set5].curvalue = cel[AnimFrame::ANGLE]
              canvas.setSpriteBitmap(previewsprite, cel)
              pbSpriteSetAnimFrame(previewsprite, cel, nil, nil)
              previewsprite.z = previewwin.z + 1
              sliderwin2.update
            ...
            when 6 # Delete Frame
              ...
              params.setInitialValue(cel[AnimFrame::OPACITY])
              params.setCancelValue(cel[AnimFrame::OPACITY])
              cel[AnimFrame::OPACITY] = Kernel.pbMessageChooseNumber('Opacity Value:', params)
              sliderwin2.controls[set6].curvalue = cel[AnimFrame::OPACITY]
              canvas.setSpriteBitmap(previewsprite, cel)
              pbSpriteSetAnimFrame(previewsprite, cel, nil, nil)
              previewsprite.z = previewwin.z + 1
              sliderwin2.update
            ...
            when 7 # Delete Frame
              params = ChooseNumberParams.new
              params.setRange(0, 2)
              params.setInitialValue(cel[AnimFrame::BLENDTYPE])
              params.setCancelValue(cel[AnimFrame::BLENDTYPE])
              cel[AnimFrame::BLENDTYPE] = Kernel.pbMessageChooseNumber('Blend Type Value:', params)
              sliderwin2.controls[set7].curvalue = cel[AnimFrame::BLENDTYPE]
              canvas.setSpriteBitmap(previewsprite, cel)
              pbSpriteSetAnimFrame(previewsprite, cel, nil, nil)
              previewsprite.z = previewwin.z + 1
              sliderwin2.update
...
def pbSelectSE(canvas, audio)
  ...
  animfiles = animfiles.sort_by { |name|
    name.include?("[Play user's cry]") ? 0 : (name.include?(".wav") ? 1 : (name.include?(".mp3") ? 2 : (name.include?(".ogg") ? 3 : 4)))
  }
...
def pbAnimationsOrganiser
  ...
  loop do
    ...
    if cmd[0] == 1 # Swap animation up
      ...
      cmd2 = Kernel.pbMessage(
        _INTL("Save changes?"),
        [_INTL("Yes"), _INTL("No"), _INTL("Cancel")],
        3
      )
      if cmd2 == 0 || cmd2 == 1
        if cmd2 == 0
