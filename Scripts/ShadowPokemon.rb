...
class PokeBattle_Pokemon
  ...
  def makeShadow
    ...
    if $cache.pkmn[self.species, @form].shadowmoves
      for i in 0...4
        self.shadowmoves[i] = 0
        self.shadowmoves[i] = $cache.pkmn[self.species, @form].shadowmoves[i] if $cache.pkmn[self.species, @form].shadowmoves[i]
      end
      self.shadowmovenum = $cache.pkmn[self.species, @form].shadowmoves.length
    else
...
class PokeBattle_Move_12C < PokeBattle_Move
  def pbEffect(attacker, opponent, hitnum = 0, alltargets = nil, showanimation = true)
    ...
    ret = opponent.pbReduceStat(PBStats::EVASION, 2, statdropper: attacker)
