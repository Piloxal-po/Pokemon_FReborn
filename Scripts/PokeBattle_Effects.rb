...
#===============================================================================
  def pbCanFreeze?(showMessages)
    ...
    return false if @battle.pbWeather==PBWeather::SUNNYDAY && !hasWorkingItem(:UTILITYUMBRELLA)
...
#===============================================================================
  ...
  def pbReduceAttackStatStageIntimidate(opponent)
    ...
    if pbCanReduceStatStage?(PBStats::ATTACK,false)
      ...
      # Battle message
      oppabilityname=PBAbilities.getName(opponent.ability)
      @battle.pbDisplay(_INTL("{1}'s {2} cuts {3}'s Attack!",opponent.pbThis, oppabilityname,pbThis(true))) if !(self.ability == PBAbilities::CONTRARY)
      @battle.pbDisplay(_INTL("{1}'s {2} boosts {3}'s Attack!",opponent.pbThis, oppabilityname,pbThis(true))) if (self.ability == PBAbilities::CONTRARY)
