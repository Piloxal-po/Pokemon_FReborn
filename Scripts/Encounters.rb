...
class PokemonEncounters
  ...
  def setup(mapID)
    ...
    begin
      if $cache.mapdata[mapID]
        ...
        @density = [
          landrate, caverate, waterrate, caverate, waterrate, waterrate,
          landrate, landrate, landrate, landrate, landrate, landrate, landrate
        ]
        @enctypes = [
          mapdata.Land.nil? ? nil : mapdata.Land, mapdata.Cave.nil? ? nil : mapdata.Cave, mapdata.Water.nil? ? nil : mapdata.Water,
    ...
    if firstpoke && rand(2) == 0
      ...
      if type != -1
        ...
        for i in 0...encounters.length
          ...
          form = pbISActuallyDifferentForm(species)
  ...
  def pbISActuallyDifferentForm(species)
    ...
    return 0 if !$cache.pkmn[species].formInit || $cache.pkmn[species].formInit.empty?

    form = eval($cache.pkmn[species].formInit).call
    return 0 if form == 0 || form.nil?
    return 0 if !$cache.pkmn[species, form]
  ...
  def pbGenerateEncounter(enctype)
    ...
    @stepcount += 1
    return nil if @stepcount <= 10 && $game_variables[:EncounterRateModifier] <= 1 # Check three steps after battle ends
