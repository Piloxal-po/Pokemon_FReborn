...
class Updater
  ...
  def self.handleMajorUpdate
    ...
    unless self.getAvailableVersion.include?('-')
      Kernel.pbMessage(_INTL("Note that you may need to be in certain location before installing this update."))
      Kernel.pbMessage(_INTL("Please check the instructions from game developers before upgrading."))
      Kernel.pbMessage(_INTL("In case you're using any major mods it's recommended to avoid this update until the mods are compatible."))
      Kernel.pbMessage(_INTL("This mainly applies to mods that are adding new Pokémon species or forms."))
    end
  end
end
