...
class Sprite_Character < RPG::Sprite
  ...
  def update
    ...
    if Reborn && !@character.graphicalNow? && @character.animation_id == 0
    ...
    if @tile_id != @character.tile_id || @character_name != @character.character_name || @character_hue != @character.character_hue || @oldbushdepth != bush_depth
      ...
      if @tile_id >= 384
        ...
        @charbitmap.dispose if @charbitmap
        @charbitmap = createCharacterBitmap
    ...
    if @tile_id == 0
      if (@character == $game_player) && ($PokemonGlobal.surfing || $PokemonGlobal.diving || $PokemonGlobal.lavasurfing)
        # Apply diving tone to surfing sprite while underwater.
        self.color.set(-120, -40, 0, 100) if $PokemonGlobal.diving && !Rejuv
        self.oy = @ch - 16
  ...
  def getRect
    return Rect.new(0, 0, 0, 0) if @character.nil? || @character.character_name.nil? || @character.character_name == ""
    ...
    sy = (@character.direction - 2) / 2 * ch
    # charbitmap.dispose
    return Rect.new(sx, sy, cw, ch)
  end

  def createCharacterBitmap
