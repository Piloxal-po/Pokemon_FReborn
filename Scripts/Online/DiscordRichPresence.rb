...
module DiscordRPC
  ...
  def self.onTrainerPartyLoad(sender, event)
    ...
    if $game_switches[:Discord_Spoilers]
      ...
      title = $cache.trainertypes[trainer.trainertype].title
      if $PokemonGlobal.partner && $PokemonGlobal.partner[0] != trainer.trainertype
        @@discordRPCInfo[:details] = "Partner: " + title + " " + trainer.name
      else
        @@discordRPCInfo[:details] = "Battling: " + title + " " + trainer.name
      end
  ...
  def self.onDoubleBattleLoad(sender, event)
    if $game_switches[:Discord_Spoilers]
      ...
      if sender == "trainer"
        opp1 = event[0][0]
        opp2 = event[1][0]
        title1 = $cache.trainertypes[opp1.trainertype].title
        title2 = $cache.trainertypes[opp2.trainertype].title
        @@discordRPCInfo[:details] = "Battling: " + title1 + " " + opp1.name + " & " + title2 + " " + opp2.name
