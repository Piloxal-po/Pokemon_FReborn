...
class SpriteWindow_Selectable < SpriteWindow_Base
  ...
  def update(canPageScroll = true)
    ...
    if self.active && @item_max > 0 && @index >= 0 && !@ignore_input
      ...
      if canPageScroll
        page_size = self.page_item_max
        page_size *= 10 if Input.press?(Input::CTRL) || (!$joiplay && Input::Controller.axes_trigger[1] >= 0.1)
        if Input.repeat?(Input::R)
          if self.index < @item_max - 1
            oldindex = @index
            @index = [self.index + page_size, @item_max - 1].min
            if @index != oldindex
        ...
        if Input.repeat?(Input::L)
          if self.index > 0
            oldindex = @index
            @index = [self.index - page_size, 0].max
            if @index != oldindex
              self.top_row -= self.page_row_max
              item_changed
            end
      ...
      if Input.triggerex?(:HOME)
        if @index != 0
          @index = 0
          item_changed
        end
      end
      if Input.triggerex?(:END)
        if @index != @item_max - 1
          @index = @item_max - 1
          item_changed
        end
...
class Window_UnformattedTextPokemon < SpriteWindow_Base
  ...
  def resizeToFitInternal(text, maxwidth) # maxwidth is maximum acceptable window width
    ...
    getLineBrokenChunks(self.contents, text, cwidth - self.borderX - SpriteWindow_Base::TEXTPADDING, dims, true)
    return dims
  end
  ...
  def refresh
    ...
    self.contents.clear
    drawTextEx(self.contents, 0, 0, self.contents.width, 0, @text.gsub(/\r/, ""), @baseColor, @shadowColor)
  end
end
...
class Window_AdvancedTextPokemon < SpriteWindow_Base
  ...
  def dispose
    ...
    @pausesprite = nil
    clearCustomFormatting() if Rejuv
    super
  end
  ...
  def resizeToFitInternal(text, maxwidth)
    dims = [0, 0]
    cwidth = maxwidth < 0 ? Graphics.width : maxwidth
    chars = getFormattedTextForDims(self.contents, 0, 0, cwidth - self.borderX - 2 - 6, -1, text, @lineHeight, true)
    for ch in chars
  ...
  def update
    super
    updateCustomFormatting() if Rejuv
...
class AnimatedSprite < SpriteWrapper
  ...
  def initializeLong(animname, framecount, framewidth, frameheight, frameskip)
    ...
    if @animbitmap.width % framewidth != 0
      raise _INTL("Bitmap's width ({1}) is not a multiple of frame width ({2}) [Bitmap={3}]", @animbitmap.width,
                  framewidth, animname)
    end
    if @animbitmap.height % frameheight != 0
      raise _INTL("Bitmap's height ({1}) is not a multiple of frame height ({2}) [Bitmap={3}]", @animbitmap.height,
                  frameheight, animname)
    end
  ...
  def initializeShort(animname, framecount, frameskip)
    ...
    if @animbitmap.width % framecount != 0
      raise _INTL("Bitmap's width ({1}) is not a multiple of frame count ({2}) [Bitmap={3}]", @animbitmap.width,
                  framewidth, animname)
    end
