...
class SpriteWindow_DebugRight < Window_DrawableCommand
  ...
  def shadowtext(x, y, w, h, t, align = 0)
    ...
    pbDrawShadowText(self.contents, x, y, [width, w].max, h, t, Color.new(12 * 8, 12 * 8, 12 * 8), Color.new(26 * 8, 26 * 8, 25 * 8))
  end
    ...
    case menuoptions[command]
        ...
        loop do
          ...
          # Set HP
            ...
            newhp = Kernel.pbMessageChooseNumber(
              _INTL("Set the Pokémon's HP (max. {1}).", pkmn.totalhp), params
            ) { @scene.update }
            if newhp != pkmn.hp
              pkmn.hp = newhp
      ...
      when "Moves"
        ...
        loop do
          cmd = @scene.pbShowCommands(
            _INTL("Do what with {1}?", pkmn.name),
            [
              _INTL("Teach move"),
              _INTL("Forget move"),
      ...
      when "Ability"
        loop do
          ...
          msg = [
            ...
            _INTL("Ability {1} is being forced.", getAbilityName(abils[cmd]))
          ][pkmn.abilityIndex == abils[cmd] ? 0 : 1]
      ...
      when "Nature"
        ...
        loop do
          ...
          msg = [
            _INTL("Nature {1} is natural.", oldnature),
            _INTL("Nature {1} is being forced.", oldnature)
          ][pkmn.natureflag ? 1 : 0]
          cmd = @scene.pbShowCommands(msg, commands, cmd)
          # Break
          if cmd == -1
        ...
        loop do
          ...
          # Make egg
            ...
            if pbHasEgg?(pkmn) ||
              ...
              pkmn.calcStats
              pkmn.name = _INTL("Egg")
              pkmn.eggsteps = $cache.pkmn[pkmn.species, pkmn.form].EggSteps
              pkmn.hatchedMap = 0
              pkmn.obtainMode = 1
              @scene.pbHardRefresh
            end
