...
class HallOfFameScene
  ...
  def writeTrainerData
    ...
    lefttext += _INTL("Pokédex<r>{1}/{2}<br>", $Trainer.pokedex.getOwnedCount, $Trainer.pokedex.getSeenCount)
  ...
  def writePokemonData(pokemon, hallNumber = -1)
    ...
    dexnumber = pokemon.isEgg? ? _INTL("No. ???") : _ISPRINTF("No. {1:03d}", pokemon.dexnum)
    textPositions = [
      ...
      [_INTL("Lv. {1}", pokemon.isEgg? ? "?" : pokemon.level), 64, Graphics.height - 48, 0, BASECOLOR, SHADOWCOLOR],
  ...
  def writeWelcome
    overlay = @sprites["overlay"].bitmap
    overlay.clear
    pbDrawTextPositions(overlay, [[_INTL("Welcome to the Hall of Fame!"), Graphics.width / 2, Graphics.height - 80, 2, BASECOLOR, SHADOWCOLOR]])
  end
