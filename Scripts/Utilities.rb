...
def pbPokemonBitmap(species, shiny = false, back = false, gender = nil, form = 0, looped = false) # Used by the Pokédex
  ...
  gendermod = gender == "Female" ? (Reborn ? "f" : "_f") : ""
  dexnum = $cache.pkmn[species, formname].dexnum
  ...
  toobig = $cache.pkmn[species, formname].checkFlag?(:toobig)
  if toobig
    backtag = back ? "b" : ""
    shinytag = shiny ? "s" : ""
  ...
  if looped
    p "#{$cache.pkmn.keys[$cache.pkmn[species, form].dexnum - 1]} form #{form}#{back ? " back" : ""} missing graphic"
    return nil
  end
  return pbPokemonBitmap(species, shiny, back, nil, form, true)
end
...
def pbLoadPokemonBitmapSpecies(pokemon, species, back = false)
  ...
  y = form * 384
  y = 8 * 384 if (species == :SILVALLY && form == 19) # Vulpes abusing her dev priviledges
  y = form * 192 if pokemon.isEgg?
  y += back ? 192 : 0
  y = 0 if toobig
...
def pbPokemonIconBitmap(pokemon, egg = false) # pbpokemonbitmap, but for icons
  begin
    ...
    form = pokemon.form
    egg = egg ? "egg" : ""
    species = $cache.pkmn[pokemon.species, pokemon.form].dexnum
...
def pbIconBitmap(species, form = 0, shiny = false, girl = false, egg = false) # pbpokemonbitmap, but for icons
  ...
  form = 0 if cancelledgenders.include?(species) || form == nil
  species = $cache.pkmn[species, form].dexnum # .name.downcase
...
def getTrainerOverworldSprite(tclass)
  ...
  if !filename
    print "hey, this trainer is missing a graphic. please report: ", tclass, " - ", filename, " ty <3"
    filename = "trchar000.png"
  end
  if !pbResolveBitmap("Graphics/Characters/" + filename)
    print "hey this trainer's graphic does not exist. please report: ", tclass, " - ", filename, " ty <3"
    filename = "trchar000.png"
  end
  return filename
end
...
def pbPlayCry(pokemon, volume = 90, pitch = nil)
  ...
  if pokemon.is_a?(Numeric)
    ...
    if pokemon.respond_to?("chatter") && pokemon.chatter
      ...
      pkmnwav = pbCryFile(pokemon)
      if pkmnwav
        pbSEPlay(RPG::AudioFile.new(pkmnwav, volume, pitch ? pitch : (pokemon.hp * 25 / pokemon.totalhp) + 75)) rescue nil
      end
    end
  end
end
...
def pbGenerateEgg(pokemon, form = 0, text = "")
  ...
  pokemon.eggsteps = $cache.pkmn[pokemon.species, form].EggSteps
...
def pbHasEgg?(pkmn)
  ...
  compat1 = $cache.pkmn[pkmn.species, pkmn.form].EggGroups # Get egg group of this species
...
def rotomScript(pokemon)
  ...
  if form > 0
    pbLearnMove(pokemon, moves[form - 1], true, false)
  end
  if pokemon.moves.find_all { |i| i.move != 0 }.length == 0
    pbLearnMove(pokemon, :THUNDERSHOCK)
  end
end
