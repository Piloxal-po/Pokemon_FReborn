...
class RandomizerScene
  ...
  def initialize(settings = RandomizerSettings.new)
    ...
    if Dir.exists?("Randomizer Data") && File.exists?("Randomizer Data/settings.txt")
    ...
    loop do
      ...
      if @confirmflag
        ...
        if !@settingsChanged
          ...
          # Dir.mkdir("Randomizer Data") unless Dir.exists?("Randomizer Data")
          if Dir.exists?("Randomizer Data")
  ...
  def makePages(viewport, sidewinlength)
    ...
    typepage.addCheckbox("Allow Shadow Type?", proc { |value| @settings.types[:allowQmarks?] = value })
