class Scene_ItemTracker
  ...
  def main
    ...
    @sprites["header"] = Window_UnformattedTextPokemon.newWithSize(_INTL("Tracker"), 2, -18, 128, 64, @viewport)
...
class Scene_ItemTracker_Info
  ...
  def initPageData
    ...
    name_to_id = {}
    for item in items
      name_to_id[getItemName(item)] = item
    end
    # sort by name
