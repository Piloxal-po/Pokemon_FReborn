      ...
      for i in 0...$PokemonStorage[iBox].length
        ...
        if poke
          ...
          if iIsNickName == 0
            ...
            sFound = getMonName(poke.species, poke.form)
          else
            if poke.item.nil?
              next
            else
      ...
      loop do
        ...
        if selected == nil
          ...
          case selected.pop
            ...
            when 4 # Multi-Move
              pbHold(selected, true) if selected[0] >= 0 && !@heldpkmn && pokemon
            when 5 # Deposit / Withdraw
              heldpoke = pbHeldPokemon
              next if !pokemon && !heldpoke
              if selected[0] == -1
...
class PokemonStorageScene
  ...
  def pbUpdateOverlay(selection, party = nil)
    ...
    pokemonstring = pokename + ", "
    pokemonstring = pokemonstring + " Level " + pokemon.level.to_s + ", "
    if pokemon.type2.nil?
      pokemonstring = pokemonstring + pokemon.type1.name + " type, "
    else
      pokemonstring = pokemonstring + pokemon.type1.name + "/" + pokemon.type2.name + " type, "
    end
    pokemonstring += "Ability: " + abilityname + ", "
    pokemonstring += "Item: " + itemname
    tts(pokemonstring)
  end
