...
def Kernel.pbShowCommands(msgwindow, commands = nil, cmdIfCancel = 0, defaultCmd = 0, tts: true)
  ret = 0
  if commands
    cmdwindow = Window_CommandPokemon.new(commands, tts: tts)
    cmdwindow.z = 99999
    cmdwindow.visible = true
    cmdwindow.resizeToFit(cmdwindow.commands)
    pbPositionNearMsgWindow(cmdwindow, msgwindow, :right)
...
def Kernel.pbMessage(message, commands = nil, cmdIfCancel = 0, skin = nil, defaultCmd = 0, tts: true, &block)
  ...
  if commands
    ret = Kernel.pbMessageDisplay(
      ...
      proc { |msgwindow|
        next Kernel.pbShowCommands(msgwindow, commands, cmdIfCancel, defaultCmd, tts: tts, &block)
      },
      &block
    )
  else
    Kernel.pbMessageDisplay(msgwindow, message, tts: tts, &block)
  end
...
def pbDisplayAPWindow(msgwindow, goldwindow)
  apString = pbCommaNumber($game_variables[526])
...
def Kernel.pbMessageDisplay(msgwindow, message, letterbyletter = true, commandProc = nil, tts: true)
  return if !msgwindow
  ...
  tts(toUnformattedText(text)) if tts
  signWaitCount = 0
  shout = false
  ...
  begin
    ...
    if $Settings.textskipwait == 1 ? Input.press?(Input::B) : Input.time?(Input::B) > 0.15
      msgwindow.textspeed = -999
      msgwindow.update
      if msgwindow.busy?
    ...
    if Input.trigger?(Input::C)
      if msgwindow.busy? && (!$game_switches || !$game_switches[:Blindstep])
        pbPlayDecisionSE() if msgwindow.pausing?
        msgwindow.resume
      else
        break if signWaitCount == 0
