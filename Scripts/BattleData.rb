...
class Battle_Data
  ...
  def initialize(opponentName, playerTeam, opponentTeam)
    ...
    @playerTeam = Marshal.load( Marshal.dump(playerTeam))
    @opponentTeam =  Marshal.load( Marshal.dump(opponentTeam))
    @oldMVPName = nil
    @lastPokemonUsed = nil
  ...
  def afterBattleStateOfPartyResult
    ...
    for i in 0..@playerTeam.length - 1
      ...
      noMonAbove25PercentHealth = false if (@playerTeam[i].hp.to_f / @playerTeam[i].totalhp.to_f) > 0.25
    end
  ...
  def acquiredZCrystal(oldMon, mvp)
    return (!(pbIsZCrystal?(oldMon.item) || pbIsZCrystal?(oldMon.item)) && \
    (pbIsZCrystal?(mvp.item) || pbIsZCrystal?(mvp.item)))
  end
  ...
  def getScoreAndSide(battler)
    ...
    for i in 0...@playerTeam.length
      if @playerTeam[i].personalID == idtofind
        return [@playerTeamScores[i], "play", @playerTeam[i].name]
      end
    end
    for j in 0...@opponentTeam.length
      if @opponentTeam[j].personalID == idtofind
        return [@opponentTeamScores[j], "opp", @opponentTeam[j].name]
      end
    end
    return 0
  end
