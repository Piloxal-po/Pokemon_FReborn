class BattleSwapScene
  ...
  def pbGetCommands(list, choices)
    ...
    for i in 0...list.length
      pkmn = list[i]
      kind = $cache.pkmn[pkmn.species, pkmn.form].kind
      selected = shadowctagFromColor(Color.new(232, 0, 0))
      if choices.any? { |item| item == i }
        commands.push(
          selected + _INTL("{1} - {2} Pokémon", getMonName(pkmn.species, pkmn.form), kind)
        )
      else
        commands.push(
          _INTL("{1} - {2} Pokémon", getMonName(pkmn.species, pkmn.form), kind)
        )
      end
    end
    return commands
  ...
  def pbUpdateChoices(choices)
    commands = pbGetCommands($rentalsnew, choices)
    @choices = choices
    if choices.length == 0
  ...
  def pbSwapChosen(pkmnindex)
    ...
    commands.push(_INTL("Pokémon for swap"))
