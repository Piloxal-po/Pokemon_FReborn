...
class TilePuzzleCursor < BitmapSprite
  ...
  def update
    ...
    if @selected || @game == 3
      expand = (@game == 3) ? 0 : 4
      xin = [
        (@tilewidth - @cursorbitmap.width / 4) / 2, -expand,
        @tilewidth - @cursorbitmap.width / 4 + expand,
        (@tilewidth - @cursorbitmap.width / 4) / 2
      ]
      yin = [
        @tileheight - @cursorbitmap.height / 2 + expand,
        (@tileheight - @cursorbitmap.height / 2) / 2,
        (@tileheight - @cursorbitmap.height / 2) / 2,
        -expand
      ]
      for i in 0...4
        if @arrows[i]
          self.bitmap.blt(
            x + xin[i], y + yin[i], @cursorbitmap.bitmap,
            Rect.new(
...
class TilePuzzleScene
  ...
  def pbStartScene
    ...
    @sprites["cursor"] =
      TilePuzzleCursor.new(@game, pbDefaultCursorPosition, @tilewidth, @tileheight, @boardwidth, @boardheight)
    update
    ...
    if @game == 6
      ...
      if @game == 3 # Make sure only solvable Mystic Squares are allowed.
        ...
        if @boardwidth % 2 == 1
          ...
          ret = pbShuffleTiles unless (num % 2 == 0 && (@boardheight - (blank / @boardwidth)) % 2 == 1) || (num % 2 == 1 && (@boardheight - (blank / @boardwidth)) % 2 == 0)
        end
      end
      if @game == 1 || @game == 2
        ret2 = []
    ...
    loop do
      ...
      if pbCheckWin
        ...
        if @game == 3
          ...
          extratile.bitmap.blt(
            0, 0, @tilebitmap.bitmap,
            Rect.new(@tilewidth * (@boardwidth - 1), @tileheight * (@boardheight - 1), @tilewidth, @tileheight)
          )
          extratile.opacity = 0
          32.times do
            extratile.opacity += 8
      ...
      elsif (@game == 2 && (Input.trigger?(Input::A) || Input.trigger?(Input::X) || Input.trigger?(Input::Y))) ||
            (@game == 5 && (Input.trigger?(Input::A) || Input.trigger?(Input::X) || Input.trigger?(Input::Y))) ||
