class PokemonSaveScene
  def pbStartScreen
    ...
    if $Trainer.postgame != nil
      loctext += _INTL("Legendaries<r><c3={1}>{2}</c3><br>", textColor, $Trainer.postgame)
    else
    ...
    if hour > 0
      loctext += _ISPRINTF("Time<r><c3={1:s}>{2:02d}h {3:02d}m</c3><br>", textColor, hour, min)
    else
      loctext += _ISPRINTF("Time<r><c3={1:s}>{2:02d}m</c3><br>", textColor, min)
    end
...
def pbSave(safesave = false)
  ...
  if $Settings.backup == 0
    ...
    if safeExists?(RTP.getSaveSlotPath($Unidata[:saveslot]))
      ...
      if $Trainer.postgame != nil
        savename += " - #{trainame} - #{number} - #{hour}h #{min}m - #{trainer.postgame} legendaries - #{mapname}.rxdata"
      else
