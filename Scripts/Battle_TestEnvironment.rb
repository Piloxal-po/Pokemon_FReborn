class PokeBattle_Battle
  ...
  def testAllMoves
    ...
    $cache.movess = load_data("Data/attacksRS.dat") if !$cache.movess
    startTime = Time.now
    1.step(694, 4) { |i|
  ...
  def pbAttackPhaseTEST
    ...
    for i in priority
      ...
      @megaEvolution[side][owner] -= 1 if side == 0 && @megaEvolution[side][owner] > 0
...
def trainershit(trainer)
  ...
  name = pbGetMessageFromHash(MessageTypes::TrainerNames, trainer[1]) # HIIIIII KILL THIS and is also probably giga broken
  ...
  for poke in trainer[3]
    ...
    if $game_switches[:Only_Pulse_2] # pulse 2 mode
      for i in 0...6
        pokemon.ev[i] = 252
      end
    end
...
def specificTrainer(startpoint = 0, filename = "Specific_Battle.txt")
  ...
  $game_variables[:Forced_Field_Effect] =
    Kernel.pbMessageChooseNumber("Choose the field effect for the battles.", params)
...
def random_battles
  ...
  begin
    loop do
      ...
      [trainer1[2], trainer2[2]].each { |trainer|
        trainer.each { |mon|
...
class PokeBattle_Scene
      ...
      if Input.trigger?(Input::C) # Confirm choice
        ...
        if cw.zButton == 2
          if battler.pbCompatibleZMoveFromMove?(ret, true)
            ...
            @battle.pbDisplay(_INTL("{1} is not compatible with {2}!", getMoveName(battler.moves[ret]), getItemName(battler.item)))
    ...
    loop do
      ...
      if cmdUse >= 0 && command == cmdUse
        if usetype == 1 || usetype == 3
          ...
          pkmnlist.pbEndScene
          itemscene.pbStartScene($PokemonBag)
        elsif (usetype == 2 && !$cache.items[item].checkFlag?(:battleitem)) || usetype == 4
  ...
  def pbDamageAnimation(pkmn, effectiveness, quick: false)
  end
...
class PokeBattle_Pokemon
  def initialize(species, level, player = nil, withMoves = true)
    ...
    group1 = $cache.pkmn[species][:EggGroups][0]
    group2 = $cache.pkmn[species][:EggGroups][1]
    time = pbGetTimeNow
    ...
    if withMoves
      ...
      movelist = []
      for k in 0...$cache.movess[species].length
        alevel = $cache.movess[species][k][0]
        move = $cache.movess[species][k][1]
        if alevel <= level
          movelist[k] = move
        end
      end
