...
def transformPoint(
  ...
  x3, y3, x4, y4,
  px, py
) ### Source line Destination line Source point
...
class PBAnimationPlayerX
  ...
  def initialize(animation, user, target, scene = nil, move = nil, oppmove = false, ineditor = false)
    ...
    if @partnerbs
      ...
      useroutofnames = [
        realusersprite.x + (realusersprite.bitmap.width / 2),
        realusersprite.y + (realusersprite.bitmap.height / 2)
      ]
      srcline = [PBScene::FOCUSUSER_X, PBScene::FOCUSUSER_Y, PBScene::FOCUSTARGET_X, PBScene::FOCUSTARGET_Y]
    ...
    if !ineditor && @animation.targets == 2
      for index in 0...4
        allmontargets.push(index) if index != @realuser.index && index != @realtarget.index && user.battle.battlers[index].hp > 0 && user.battle.battlers[index].vanished == false
      end
  ...
  def update
    ...
    if !@animbitmap || @animbitmap.disposed?
      @animbitmap = AnimatedBitmap.new("Graphics/Animations/" + @animation.graphic, @animation.hue).deanimate
