...
class PokemonNestMapScene
  def pbStartScene(species, regionmap = -1)
    ...
    for mapid in 1...$cache.mapdata.length
      if pbFindEncounter(mapid, species)
        ...
        if mappos && mappos[0] == region
          ...
          # end
          enclist.push($cache.mapinfos[mapid].name)
          if showpoint
    ...
    enclist = shortEncounterList(enclist)
    i = 0
    ...
    @sprites["mapbottom"].mapdetails = _INTL("{1}'s nest", getMonName(species, $Trainer.pokedex.dexList[species][:lastSeen][:form]))
    tts("#{getMonName(species)}'s nest:")
    if points.length == 0
...
class PokedexFormScene
  ...
  def pbUpdate
    ...
    nametext = getMonName(@species, @form)
    ...
    backMetric = $cache.pkmn[@species, @form].BattlerPlayerY
  ...
  def pbGetAvailable
    ...
    genderbyte = $cache.pkmn[@species, @form].GenderRatio
    ...
    for forms in formnames
      availableForms.push(forms) if $Trainer.pokedex.dexList[@species][:forms][forms] && (forms == formnames[0] || !$cache.pkmn[@species, @form].checkFlag?(:ExcludeDex))
    end
  ...
  def pbChooseForm
    ...
    oldform = @form
    choicearr = []
    choicearr.push("Gender") if $Trainer.pokedex.dexList[@species][:gender].length > 1
    choicearr.push("Form") if $Trainer.pokedex.dexList[@species][:forms].length > 1
    ...
    if choicearr[choice] == "Toggle Shiny"
      @shiny = !@shiny
      pbUpdate
    end
  end
