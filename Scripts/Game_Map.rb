class Game_Map
  ...
  def setup(map_id, fromLoad = false, mapdata = nil)
    ...
    @snap_edges = $cache.mapdata[@map_id].SnapEdges
  ...
  def autoplayAsCue
    if @map.autoplay_bgm
      ...
      newnamecheck = false
      if newname.include?("Reborn- ") && $game_switches[:Reborn_City_Restore]
        newnamecheck = true
      ...
      elsif $game_switches[:Reborn_City_Restore] && newnamecheck && FileTest.audio_exist?("Audio/BGM/" + newname) # Post-renovation city music
  ...
  def autoplay(from_startup = false)
    ...
    if from_startup && $game_system.saved_bgm
      # $game_system.saved_bgm may have incorrect volume so we only use it in case it's different from autoplay
      unless @map.autoplay_bgm && @map.bgm && @map.bgm.name == $game_system.saved_bgm.name
        bgm = $game_system.saved_bgm
        $game_system.bgm_play(nil)
        pbBGMPlay(bgm, reset_volume: true)
        return
      else
        pbBGMStop
      end
    ...
    if @map.autoplay_bgm
      ...
      elsif $game_switches[:Reborn_City_Restore] && newnamecheck &&
  ...
  def playerPassable?(x, y, d, self_event = nil)
    ...
    [2, 1, 0].each { |i|
      ...
      # Make water tiles passable if player is surfing
        ...
        return true
      elsif $PokemonGlobal.lavasurfing && pbIsPassableLavaTag?(@terrain_tags[tile_id])
        return true
      # Prevent cycling in really tall grass
      elsif $PokemonGlobal.bicycle && (@terrain_tags[tile_id] == PBTerrain::TallGrass || @terrain_tags[tile_id] == PBTerrain::SandDune)
        return false
      # Prevent Taurosing in really tall grass
      elsif Kernel.pbFacingTerrainTag == PBTerrain::TallGrass && $game_switches[:Riding_Tauros] && !$game_switches[:Blindstep] && self_event == $game_player
        return false
      # Prevent cycling on ice
      elsif $PokemonGlobal.bicycle && @terrain_tags[tile_id] == PBTerrain::Ice
        return false
      # Regular passability checks
      elsif @passages[tile_id] & bit != 0
        return false
  ...
  def name
    ret = $cache.mapinfos[self.map_id].name
    if $Trainer
      ret = ret.gsub(/\\PN/, $Trainer.name)
    end
    return ret
  end
