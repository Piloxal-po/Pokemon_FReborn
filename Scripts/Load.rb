class PokemonLoadPanel < SpriteWrapper
  ...
  def refresh
    ...
    if @refreshBitmap
      ...
      elsif @title.start_with?("New Patch")
  ...
  def pbStartScene(commands, showContinue, trainer, framecount, mapid)
    ...
    if GAMEVERSION != 'dev'
      ...
      if version.index("-") != nil
        ...
        @sprites["overlay2"].bitmap.draw_text(-4, Graphics.height - 46, Graphics.width, 30, version, 2)
        @sprites["overlay2"].bitmap.draw_text(-4, Graphics.height - 28, Graphics.width, 30, prerelease, 2)
      else
        @sprites["overlay2"].bitmap.draw_text(-4, Graphics.height - 28, Graphics.width, 30, version, 2)
      end
    end
...
class PokemonLoad
  ...
  def pbStartLoadScreen
    ...
    if Updater.shouldUseBuiltInUpdater?
      ...
      if versionStatus == 3
        commands[cmdPatchUpdate = commands.length] = _INTL("New Patch: {1}", Updater.getAvailableVersion)
    ...
    commands[cmdSaveDir = commands.length] = _INTL("Open Save Folder") unless $joiplay
