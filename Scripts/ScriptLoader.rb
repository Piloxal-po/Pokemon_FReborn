...
def fileExists?(file)
  # System.file_exist? respects paths defined in "patches" directive in mkxp.json, unlike File.exists?.
  return System.file_exist?(file) unless $joiplay
  # JoiPlay however doesn't use mkxp.json, nor does it have a System.file_exist? so we need to use ugly workaround.
  return File.exists?(file) || File.exists?("patch/"+file)
end
