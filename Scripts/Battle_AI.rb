...
class PokeBattle_AI
  ...
  PARTNERFUNCTIONS = [0x40, 0x41, 0x55, 0x63, 0x66, 0x67, 0xA0, 0xC1, 0xDF, 0x142, 0x162, 0x164, 0x167, 0x169, 0x170, 0x11d, 0x185, 0x317]
  ...
  def setUpDebugLog()
    ...
    trainame = $PokemonGlobal.partner && @battle.doublebattle ? "#{$Trainer.name} and #{@battle.player[1].fullname}" : $Trainer.name
    trainame = trainame.gsub(/[^0-9A-Za-z ]/, '')
    fieldname = PokeBattle_Field.getFieldName(@battle.FE)
    oppname = @battle.opponent.kind_of?(Array) ? "#{@battle.opponent[0].fullname} and #{@battle.opponent[1].fullname}" : @battle.opponent.fullname
    battleformat = @battle.doublebattle ? "Doubles" : "Singles"
  ...
  def buildMoveScores
    ...
    if @battle.doublebattle	# this JUST gets the numbers. other things can be computed later.
      for monindex in 0...@battle.battlers.length
        ...
        $ai_log_data[@index].expected_damage_name.push(getMonName(@opponent.species, @opponent.form)) unless monindex == @attacker.pbPartner.index || !$INTERNAL
      ...
      if @mondata.shouldswitchscore > maxmovescore && @mondata.switchscore.max > 100 # arbitrary
        if battler.index == 3 && @battle.choices[1][0] == 2 && @battle.choices[1][1] == @mondata.switchscore.index(@mondata.switchscore.max)
          if @mondata.switchscore.max(2)[1] > 100 && shouldHardSwitch?(battler, @mondata.switchscore.index(@mondata.switchscore.max(2)[1]))
            ...
            PBDebug.log(sprintf("Switching to %s", getMonName(highestscoremon.species, highestscoremon.form))) if $INTERNAL
            $ai_log_data[battler.index].chosen_action = sprintf("Switching to %s", getMonName(highestscoremon.species, highestscoremon.form))
        ...
        case battler.pbTarget(move)
          when :SingleNonUser, :SingleOpposing
            [oi, pi, ci].each do |targetindex|
              chooseablemoves.push({ moveindex: moveindex, target: [targetindex], score: mondata.scorearray[targetindex][moveindex], zmove: false })
            end
  ...
  def getMoveScore(initialscores = [], scoreindex = -1)
    ...
    if @mondata.skill >= MEDIUMSKILL && !moldBreakerCheck(@attacker) &&
       (
         (@attacker.pbTarget(@move) == :SingleNonUser || [:RandomOpposing, :AllOpposing, :OpposingSide, :SingleOpposing].include?(@attacker.pbTarget(@move)) ||
         (@attacker.pbTarget(@move) == :OppositeOpposing && @attacker.hasType?(:GHOST))) && @move.basedamage == 0
       )
      if wondercheck
    ...
    if @move.pbIsPriorityMoveAI(@attacker) && @attacker != @opponent.pbPartner
      ...
      if (@battle.doublebattle || (@opponent.status != :SLEEP && @opponent.status != :FROZEN && !@opponent.effects[:Truant] && @opponent.effects[:HyperBeam] == 0)) && !seedProtection?(@attacker) # This line might be in the wrong place, but we're trying our best here-- skip priority if opponent is incapacitated
        ...
        opppri2 = false
        pridam2 = -1
        if !aifaster || aifaster_partner === false ||
           getAIMemory().any? { |moveloop| moveloop != nil && moveloop.basedamage > 0 && moveloop.pbIsPriorityMoveAI(@opponent) } ||
           getAIMemory(@opponent.pbPartner).any? { |moveloop| moveloop != nil && moveloop.basedamage > 0 && moveloop.pbIsPriorityMoveAI(@opponent.pbPartner) } && @battle.doublebattle
    ...
    if @mondata.skill >= HIGHSKILL && @opponent.index != @attacker.index
      for j in getAIMemory(@opponent)
        ...
        shieldcheck = true if j.move == :SPIKYSHIELD || j.move == :KINGSSHIELD || j.move == :BANEFULBUNKER || j.move == :OBSTRUCT
      end
      if @battle.doublebattle
    ...
    switchableparty = @battle.pbParty(@opponent.index).find_all.with_index { |mon, monindex|
      @battle.pbCanSwitch?(@opponent.index, monindex, false, true)
    }
    ...
    if @mondata.skill >= MEDIUMSKILL && @attacker.ability == :PRANKSTER && (@opponent.hasType?(:DARK) && @battle.FE != :BEWITCHED)
      if @move.basedamage == 0 && @move.priority > -1 && (@attacker.pbTarget(@move) == :SingleNonUser || [:RandomOpposing, :AllOpposing, :OpposingSide, :SingleOpposing].include?(@attacker.pbTarget(@move)))
        $ai_log_data[@attacker.index].final_score_moves.push(0)
        return 0
      end
    end
    ...
    case @move.function
      ...
      when 0x2f # Iron Defense, Acid Armor, Barrier, Diamond Storm
        ...
        if @mondata.skill >= BESTSKILL && (@move.move == :IRONDEFENSE && @battle.FE == :FACTORY) ||
           (@move.move == :ACIDARMOR && [:CORROSIVE, :CORROSIVEMIST, :MURKWATERSURFACE, :FAIRYTALE, :CONCERT1, :CONCERT2, :CONCERT3, :CONCERT4].include?(@battle.FE))
      ...
      when 0xce # Sky Drop
        ...
        miniscore = 0 if @battle.state.effects[:Gravity] != 0 || @battle.FE == :CAVE || weight >= 2000
      ...
      when 0xd0 # Whirlpool
        miniscore = firespincode()
        miniscore *= 1.3 if !$cache.moves[opponent.effects[:TwoTurnAttack]].nil? && $cache.moves[@opponent.effects[:TwoTurnAttack]].function == 0xCB
        miniscore *= 0.7 if @battle.FE == :ASHENBEACH
      ...
      when 0xd8 # Synthesis, Moonlight, Morning Sun
        ...
        recoveramount = (@attacker.totalhp * 0.75).floor  if @mondata.skill >= BESTSKILL && (
          ([:DARKCRYSTALCAVERN, :STARLIGHT, :NEWWORLD, :BEWITCHED].include?(@battle.FE) && @move.move == :MOONLIGHT) || (Rejuv && @battle.FE == :GRASSY && @move.move == :SYNTHESIS)
        )
        miniscore = recovercode(recoveramount)
      when 0xd9 # Rest
        miniscore = restcode()
      ...
      when 0x159 # Hyperspace Fury
        if (@attacker.species == :HOOPA && @attacker.form == 1) || (Rejuv && @attacker.species == :GARDEVOIR && @attacker.form == 3) # Hoopa-U
          miniscore = nevermisscode(initialscores[scoreindex])
          miniscore *= feintcode()
          if @attacker.ability == :CONTRARY
            miniscore *= selfstatboost([0, 1, 0, 0, 0, 0, 0])
      ...
      when 0x17E # Dragon Darts
        if !@battle.doublebattle || @move.pbDragonDartTargetting(@attacker).length < 2
          ...
          miniscore = 1.2 if checkAImoves(PBStuff::PROTECTMOVE) || pbPartyHasType?(:FAIRY, @opponent.index)
        end
      when 0x17F # teatime
  ...
  def sleepcode
    return @move.basedamage > 0 ? 1 : 0 if !(@opponent.pbCanSleep?(false, false, false, moldBreakerCheck(@attacker)) && @opponent.effects[:Yawn] == 0)
  ...
  def poisoncode
    return @move.basedamage > 0 ? 1 : 0 if !@opponent.pbCanPoison?(false, false, @move.move == :TOXIC && @attacker.ability == :CORROSION, moldBreakerCheck(@attacker))
    return @move.basedamage > 0 ? 1 : 0 if hydrationCheck(@opponent)
    return @move.basedamage > 0 ? 1 : 0 if secondaryEffectNegated?()
  ...
  def paracode
    return @move.basedamage > 0 ? 1 : 0 if !@opponent.pbCanParalyze?(false, moldBreakerCheck(@attacker))
    return @move.basedamage > 0 ? 1 : 0 if hydrationCheck(@opponent)
    return @move.basedamage > 0 ? 1 : 0 if secondaryEffectNegated?()
  ...
  def burncode
    return @move.basedamage > 0 ? 1 : 0 if !@opponent.pbCanBurn?(false, false, moldBreakerCheck(@attacker))
    return @move.basedamage > 0 ? 1 : 0 if hydrationCheck(@opponent)
    return @move.basedamage > 0 ? 1 : 0 if secondaryEffectNegated?()
  ...
  def freezecode
    return @move.basedamage > 0 ? 1 : 0 if !@opponent.pbCanFreeze?(false, moldBreakerCheck(@attacker))
    return @move.basedamage > 0 ? 1 : 0 if hydrationCheck(@opponent)
    return @move.basedamage > 0 ? 1 : 0 if secondaryEffectNegated?()
  ...
  def focusenergycode
    return 0 if @attacker.effects[:FocusEnergy] > 1
  ...
  def volcanoeruptioncode
    ...
    miniscore = 1.0
    if PBTypes.twoTypeEff(:FIRE, @attacker.type1, @attacker.type2) == 0 ||
       [:MAGMAARMOR, :FLASHFIRE, :FLAREBOOST, :BLAZE, :FLAMEBODY, :SOLIDROCK, :STURDY, :BATTLEARMOR, :SHELLARMOR, :WATERBUBBLE, :MAGICGUARD, :WONDERGUARD, :PRISMARMOR].include?(@attacker.ability) ||
       @attacker.effects[:AquaRing]
      miniscore *= 1.0
    ...
    if !@attacker.pbPartner.pokemon.nil?
      if PBTypes.twoTypeEff(:FIRE, @attacker.pbPartner.type1, @attacker.pbPartner.type2) == 0 ||
         [:MAGMAARMOR, :FLASHFIRE, :FLAREBOOST, :BLAZE, :FLAMEBODY, :SOLIDROCK, :STURDY, :BATTLEARMOR, :SHELLARMOR, :WATERBUBBLE, :MAGICGUARD, :WONDERGUARD, :PRISMARMOR].include?(@attacker.pbPartner.ability) ||
         @attacker.pbPartner.effects[:AquaRing]
        miniscore *= 1.0
  ...
  def typechangecode(type)
    ...
    miniscore = [
      PBTypes.twoTypeEff(@opponent.type1, @attacker.type1, @attacker.type2),
      PBTypes.twoTypeEff(@opponent.type2, @attacker.type1, @attacker.type2)
    ].max
    minimini = [
      @opponent.type1.nil? ? 0 : PBTypes.oneTypeEff(@opponent.type1, type),
      @opponent.type2.nil? ? 0 : PBTypes.oneTypeEff(@opponent.type2, type)
    ].max
  ...
  def opptypechangecode(type)
    ...
    return 0 if @opponent.ability == :MULTITYPE || @opponent.ability == :RKSSYSTEM || @opponent.crested == :SILVALLY || @opponent.ability == :PROTEAN || @opponent.ability == :LIBERO || @opponent.ability == :COLORCHANGE || (@opponent.ability == :DOWNLOAD && @battle.FE == :DIMENSIONAL)

    miniscore = [
      PBTypes.twoTypeEff(@attacker.type1, @opponent.type1, @opponent.type2),
      PBTypes.twoTypeEff(@attacker.type2, @opponent.type1, @opponent.type2)
    ].max
    minimini = [
      PBTypes.oneTypeEff(@attacker.type1, type),
      PBTypes.oneTypeEff(@attacker.type2, type)
    ].max
  ...
  def counterattackcode
    ...
    miniscore *= 0.7 if !$cache.moves[@attacker.lastMoveUsed].nil? && $cache.moves[@attacker.lastMoveUsed].function == @move.function
    ...
    if @move.function == 0x71 # Counter
      ...
      miniscore *= 1.1 if !$cache.moves[@attacker.lastMoveUsed].nil? && $cache.moves[@attacker.lastMoveUsed].function == 0x72
      ...
      miniscore *= 1.1 if !$cache.moves[@attacker.lastMoveUsed].nil? && $cache.moves[@attacker.lastMoveUsed].function == 0x71
    end
    return miniscore
  end
  ...
  def covetcode
    return @move.basedamage > 0 ? 1 : 0 if @opponent.ability == :STICKYHOLD && !moldBreakerCheck(@attacker)
    return @move.basedamage > 0 ? 1 : 0 if @opponent.item.nil? || @battle.pbIsUnlosableItem(@opponent, @opponent.item) || (@attacker.item && !([:TRICK,:SWITCHEROO].include?(@move.move)))
    return @move.basedamage > 0 ? 1 : 0 if @opponent.effects[:Substitute] > 0
  ...
  def bestowcode
    return 0 if @opponent.ability == :STICKYHOLD && !moldBreakerCheck(@attacker)
  ...
  def suckercode
    ...
    if pbAIfaster?()
      ...
      miniscore *= 0 if @battle.FE == :CHESS && @opponent.pokemon.piece == :KING
  ...
  def rotocode(score)
    ...
    if @battle.ProgressiveFieldCheck(PBFields::FLOWERGARDEN)
      ...
      miniscore *= 1 - 0.05 * ministat if ministat > 0
      miniscore *= 1.3 if checkAIhealing()
      miniscore *= 1.5 if @attacker.pbSpeed > pbRoughStat(@opponent, PBStats::SPEED) && @battle.trickroom == 0
      miniscore *= 1.3 if @mondata.roles.include?(:SWEEPER)
      miniscore *= 0.5 if @attacker.status == :PARALYSIS
      miniscore *= 0.3 if checkAImoves([:FOULPLAY])
  ...
  def perishcode
    ...
    miniscore = 1.0
    miniscore *= 1.5 if @attacker.pbHasMove?(:UTURN) || @attacker.pbHasMove?(:VOLTSWITCH) || @attacker.pbHasMove?(:PARTINGSHOT) || @attacker.pbHasMove?(:FLIPTURN)
    ...
    miniscore *= 0.1 if (PBStuff::TRAPPINGABILITIESAI.include?(@opponent.ability) || (@opponent.ability == :MAGNETPULL && @attacker.hasType?(:STEEL)) || @attacker.effects[:MeanLook] > 0) && !(@attacker.pbHasMove?(:UTURN) || @attacker.pbHasMove?(:VOLTSWITCH) || @attacker.pbHasMove?(:PARTINGSHOT) || @attacker.pbHasMove?(:FLIPTURN))
  ...
  def spotlightcode
    ...
    elsif @opponent.crested == :SKUNTANK
  ...
  def getStatusDamage(move = @move)
    return 20 if move.zmove && (move.move == :CONVERSION || move.move == :SPLASH || move.move == :CELEBRATE || move.move == :HAPPYHOUR)
  ...
  def hpGainPerTurn(attacker = @attacker)
    ...
    if attacker.ability != :MAGICGUARD && !(attacker.ability == :WONDERGUARD && @battle.FE == :COLOSSEUM)
      ...
      healing -= (Rejuv && @battle.FE == :DESERT) ? 0.125 : 0.0625 if @battle.pbWeather == :SANDSTORM && (
        !(attacker.hasType?(:GROUND) || attacker.hasType?(:ROCK) || attacker.hasType?(:STEEL) || [:SANDFORCE, :SANDRUSH, :SANDVEIL, :MAGICGUARD, :OVERCOAT, :TEMPEST].include?(attacker.ability)) || attacker.effects[:DesertsMark]
      )
      healing -= (@battle.FE == :FROZENDIMENSION) ? 0.125 : 0.0625 if @battle.pbWeather == :HAIL && !(attacker.hasType?(:ICE) || [:SNOWCLOAK, :ICEBODY, :LUNARIDOL, :SLUSHRUSH, :MAGICGUARD, :OVERCOAT, :TEMPEST].include?(attacker.ability) || attacker.crested == :EMPOLEON)
    ...
    if attacker.ability == :DRYSKIN
      healing += 0.0625 if ((@battle.FE == :CORROSIVE || @battle.FE == :CORRUPTED) && attacker.hasType?(:POISON) && !attacker.hasType?(:STEEL)) ||
                           @battle.pbWeather == :RAINDANCE || [:MISTY, :SWAMP, :WATERSURFACE, :UNDERWATER].include?(@battle.FE) || @battle.state.effects[:MISTY] > 0
    end
  ...
  def getFieldDisruptScore(attacker = @attacker, opponent = @opponent, fieldeffect = @battle.FE, violent = false)
    ...
    case fieldeffect
      ...
      when :CORROSIVEMIST # Corromist field
        if violent
          if !PBStuff::INVULEFFECTS.any? { |eff| opponent.effects[eff] } &&
             !(PBStuff::TWOTURNMOVE.include?(opponent.effects[:TwoTurnAttack]) && pbAIfaster?(nil, nil, attacker, opponent)) &&
  ...
  def getSwitchInScoresParty(hard_switch)
    ...
    for partyindex in 0...party.length
      ...
      PBDebug.log(sprintf("Scoring for %s switching to: %s", getMonName(@attacker.species, @attacker.form), getMonName(i.species, i.form))) if $INTERNAL
      if hard_switch
        if !@battle.pbCanSwitch?(@attacker.index, partyindex, false)
          partyScores.push(-10000000)
          PBDebug.log(sprintf("Score: -10000000\n")) if $INTERNAL
          $ai_log_data[@attacker.index].switch_scores.push(-10000000)
          $ai_log_data[@attacker.index].switch_name.push(getMonName(i.pokemon.species, i.pokemon.form))
          next
        end
      ...
      pbFormChangeUponSwitchIn(nonmegaform)
      # Mega Evolution
  ...
  def shouldSwitch?
    ...
    PBDebug.log(sprintf("%s: initial switchscore: %d", getMonName(@attacker.species, @attacker.form), switchscore)) if $INTERNAL
    statantiscore = 0
    specialmove = false
    physmove = false
    for i in @attacker.moves
    ...
    if pbAIfaster?(nil, nil, @attacker, @opponent) && pbAIfaster?(nil, nil, @attacker, @opponent.pbPartner)
      betterswitchscore += 90 if @attacker.pbHasMove?(:VOLTSWITCH) || @attacker.pbHasMove?(:UTURN) || @attacker.pbHasMove?(:FLIPTURN)
    end
    ...
    PBDebug.log(sprintf("%s: initial noswitchscore: %d", getMonName(@attacker.species, @attacker.form), noswitchscore)) if $INTERNAL
  ...
  def pbBetterBaseDamage(move = @move, attacker = @attacker, opponent = @opponent)
    ...
    case move.function
      ...
      when 0x75, 0x12D # Surf, Shadow Storm
        return basedamage * 2 if !$cache.moves[opponent.effects[:TwoTurnAttack]].nil? && $cache.moves[opponent.effects[:TwoTurnAttack]].function == 0xCB # Dive
      when 0x76 # Earthquake
        return basedamage * 2 if !$cache.moves[opponent.effects[:TwoTurnAttack]].nil? && $cache.moves[opponent.effects[:TwoTurnAttack]].function == 0xCA # Dig
  ...
  def pbFormChangeUponSwitchIn(i)
    ...
    # Cherrim
    if pkmn.species == :CHERRIM && pkmn.ability == :FLOWERGIFT && (
        i.crested == :CHERRIM ||
        (@battle.pbWeather == :SUNNYDAY && !i.hasWorkingItem(:UTILITYUMBRELLA)) ||
        @battle.ProgressiveFieldCheck(PBFields::FLOWERGARDEN) ||
        @battle.FE == :BEWITCHED
      )
      i.form = 1
    end
    ...
    if pkmn.species == :DARMANITAN && (pkmn.ability == :ZENMODE || i.crested == :DARMANITAN)
      ...
      i.form = 3 if i.form == 2 && (@battle.FE == :BURNING || @battle.FE == :SUPERHEATED)
    ...
    if pkmn.species == :WISHIWASHI && pkmn.ability == :SCHOOLING
      if (i.hp > i.totalhp / 4 && i.level >= 20) || [:WATERSURFACE, :UNDERWATER, :MURKWATERSURFACE].include?(@battle.FE)
        i.form = 1
      else
        i.form = 0
      end
  ...
  def seedProtection?(battler = @attacker)
    return battler.effects[:Protect] == :KingsShield || battler.effects[:Protect] == :BanefulBunker || battler.effects[:Protect] == :SpikyShield
  end
