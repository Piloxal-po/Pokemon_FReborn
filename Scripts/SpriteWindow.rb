...
def pbResolveBitmap(x)
  ...
  RTP.eachPathFor(noext) { |path|
    filename = path + ".png" if fileExists?(path + ".png")
    filename = path + ".gif" if fileExists?(path + ".gif")
...
class SpriteWindow < Window
  ...
  def privRefresh(changeBitmap = false)
    ...
    if haveskin
      ...
      @sprites["pause"].visible = supported && @visible && @pause && (@combat & CompatBits::ShowPause)
      @sprites["cursor"].visible = supported && @visible && @openness == 255 && (@combat & CompatBits::ShowCursor)
      ...
      for i in 0..3
        ...
        if sideRects[i].width > 0 && sideRects[i].height > 0
          if (@compat & CompatBits::StretchSides) > 0 && @skinformat == 0
            ...
            tileBitmap(@sidebitmaps[i], @sprites["side#{i}"].src_rect, @_windowskin, sideRects[i])
          end
        end
      end
      if (@compat & CompatBits::ExpandBack) > 0 && @skinformat == 0
