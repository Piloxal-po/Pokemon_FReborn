  ...
  def whatfieldeffect                           #gives information depending on round and style
    if (@stylechallenge == 1) || ((@stylechallenge == 2) && (@battleNumber ==1))
      ...
      when 0
        "no field no life"
      when 4, 23, 25
        "The battle will take place in the " + pbGetMessage(MessageTypes::FieldName, $game_variables[:Forced_Field_Effect])
      when 22
        "The battle will take place Underwater"
      else
        "The battle will take place on the " + pbGetMessage(MessageTypes::FieldName, $game_variables[:Forced_Field_Effect])
      end
