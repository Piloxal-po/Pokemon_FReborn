...
class Window_PokemonBag < Window_DrawableCommand
  ...
  def itemCount
    return @bag.getPocketItems(@pocket).length + 1
  end

  def pocket=(value)
    @pocket = value
    @item_max = @bag.getPocketItems(@pocket).length + 1
  ...
  def item
    item = @bag.getPocketItems(@pocket)[self.index]
...
class PokemonBag_Scene
  ...
  def pbDetermineTMmenu(itemwindow)
    if itemwindow.item.is_a?(Symbol) && (itemwindow.pocket == TMPOCKET || pbIsTM?(itemwindow.item))
      machine = pbGetTM(itemwindow.item)
      ...
      loop do
        ...
        if Input.trigger?(Input::X)
          if pbHandleSortByType(itemwindow.pocket) # Returns true if the default sorting should be used
            pocket  = @bag.getPocketItems(itemwindow.pocket)
            counter = 1
        ...
        if Input.trigger?(Input::Y)
          thispocket = @bag.getPocketItems(itemwindow.pocket)
          if itemwindow.index < thispocket.length && thispocket.length > 1 && !POCKETAUTOSORT[itemwindow.pocket]
  ...
  def pbGetSortOrderByType
    # Item ids
    tmx, tm, memory, mint, fossil, evup, gems, utilityhold, levelup, nectar, berry, zcrystals, crests, legendhold, application, pokehold, megastones, pprestore, consumehold, keys, story, legendary, questitem, important, incense, plate, evoitem, justsell, niche, overworld, revival, status, general, battlehold, sidequest, healing, typeboost, ball, mail, battleitem, pinchberry, resistberry, evostone, evcard = Array.new(44) {
      []
    }
    $cache.items.keys.each { |item|
      ...
      typeboost.push(item) if $cache.items[item].checkFlag?(:typeboost) && !($cache.items[item].checkFlag?(:incense) || $cache.items[item].checkFlag?(:gem) || $cache.items[item].checkFlag?(:plate))
      plate.push(item) if $cache.items[item].checkFlag?(:plate)
      memory.push(item) if $cache.items[item].checkFlag?(:memory)
      questitem.push(item) if $cache.items[item].checkFlag?(:questitem)
      gems.push(item) if $cache.items[item].checkFlag?(:gem)
      ...
      tm.push(item) if pbGetTM(item) && !$cache.items[item].name.to_s.include?("TMX")
      tmx.push(item) if pbGetTM(item) && $cache.items[item].name.to_s.include?("TMX")
      ...
      crests.push(item) if $cache.items[item].checkFlag?(:crest)
    ...
    tmx2 = tmx.sort { |tmx2, tmx3|
      $cache.items[tmx2].name.sub("TMX", "").to_i <=> $cache.items[tmx3].name.sub("TMX", "").to_i
    }
    tms2 = tm.sort { |tm2, tm3|
      $cache.items[tm2].name.sub("TM", "").to_i <=> $cache.items[tm3].name.sub("TM", "").to_i
    }
...
class PokemonBag
  ...
  def rearrange()
    ...
    # Move battle items into medicine
    if @pockets[FAVORITEPOCKET].length > 0
      @pockets[2] = @pockets[2] + @pockets[FAVORITEPOCKET]
      @pockets[FAVORITEPOCKET] = []
    end
  end
  ...
  def getChoice(pocket)
    rearrange()
    return [@choices[pocket], getPocketItems(pocket).length].min || 0
  end
  ...
  def setChoice(pocket, value)
    rearrange()
    @choices[pocket] = value if value <= getPocketItems(pocket).length
  end
  ...
  def getPocketItems(pocket)
    if pocket == FAVORITEPOCKET
      items = []
      (1..8).each do |i|
        @pockets[i].each do |item|
          items.push(item) if isFavorite?(item)
        end
      end
      return items
    end
...
class PokemonBagScreen
  ...
  def pbStartScreen
    ...
    loop do
      ...
      commands[cmdUse = commands.length] =
        _INTL("Use") if ItemHandlers.hasOutHandler(item) || (pbIsTM?(item) && $Trainer.party.length > 0)
      ...
      if @bag.favoriteItems.include?(item)
...
class ItemStorageScene
  ...
  def pbStartScene(bag)
    ...
    # Item description
    @sprites["itemtextwindow"] =
      Window_UnformattedTextPokemon.newWithSize("", 84, 270, Graphics.width - 84, 128, @viewport)
    @sprites["itemtextwindow"].baseColor = ITEMTEXTBASECOLOR
    @sprites["itemtextwindow"].shadowColor = ITEMTEXTSHADOWCOLOR
    @sprites["itemtextwindow"].windowskin = nil
    @sprites["helpwindow"] = Window_UnformattedTextPokemon.new("")
  ...
  def pbRefresh
    ...
    @sprites["itemtextwindow"].text = (itemwindow.item.nil?) ? _INTL("Close storage.") : getItemDescription(itemwindow.item)
    itemwindow.refresh
  end
