...
class Scene_TimeWeather
  ...
  def main
    ...
    def getWeatherNow(weatherVar)
      return [
        _INTL("Clear"), _INTL("Rain"), _INTL("Thunderstorm"), _INTL("Snow"), _INTL("Sandstorm"),
        _INTL("HarshSun"), _INTL("Windy"), _INTL("HeavyRain"), _INTL("Blizzard"), _INTL("Unknown")
      ][weatherVar]
    end
    ...
    !outdoor ? @nextWeatherType = _INTL("Unknown") : @nextWeatherType = getWeatherNext($game_variables[:Set_Weather_1])
    ...
    tts("Weather at " + Time.at($game_variables[:Set_Weather_2]).strftime("%H:%M") +": " + @nextWeatherType.to_s)
    @selection = 0
  ...
  def pbTimeText
    ...
    presentTime = pbGetTimeNow.strftime("%H:%M")
    todaysDate = pbGetTimeNow.strftime("%a  %e  %b")
    ...
    weatherChangeTime = Time.at($game_variables[:Set_Weather_2]).strftime("%H:%M")
  ...
  def updateCustom
    ...
    if @selection == 1
      ...
      if update
        tts("Weather at " + Time.at($game_variables[:Set_Weather_2]).strftime("%k:%M") +": " + @nextWeatherType.to_s)
      end
    end
