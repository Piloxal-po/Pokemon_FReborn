  ...
  def priorityBlockedForAllTargetMoves?(attacker)
    ...
    @battle.battlers.each do |battler|
      if battler != attacker && battler != attacker.pbPartner
        if !battler.moldbroken && (battler.ability == :QUEENLYMAJESTY || battler.ability == :DAZZLING || (@battle.FE == :STARLIGHT && battler.ability == :MIRRORARMOR))
...
class PokeBattle_Move_05D < PokeBattle_Move
  def pbEffect(attacker, opponent, hitnum = 0, alltargets = nil, showanimation = true)
    ...
    if $cache.moves[newmove.move].type == :SHADOW # Shadow moves, exclusive to Shadow Pokemon
      @battle.pbDisplay(_INTL("But it failed!"))
      return -1
    end
    for i in attacker.moves
...
class PokeBattle_Move_064 < PokeBattle_Move
  def pbEffect(attacker, opponent, hitnum = 0, alltargets = nil, showanimation = true)
    ...
    if opponent.effects[:Illusion] != nil
      ...
      opponent.effects[:Illusion] = nil
      @battle.scene.pbChangePokemon(opponent, opponent.pokemon)
      @battle.pbDisplay(_INTL("{1}'s {2} was broken!", opponent.pbThis, getAbilityName(:ILLUSION)))
    end
    @battle.neutralizingGasDisable(opponent.index) if neutralgas
...
class PokeBattle_Move_069 < PokeBattle_Move
  def pbEffect(attacker, opponent, hitnum = 0, alltargets = nil, showanimation = true)
    ...
    attacker.moves.each { |copiedmove| @battle.ai.addMoveToMemory(attacker, copiedmove) }
    opponent.moves.each { |moveloop| @battle.ai.addMoveToMemory(opponent, moveloop) }
...
class PokeBattle_Move_09F < PokeBattle_Move
  ...
  def pbShowAnimation(id, attacker, opponent, hitnum = 0, alltargets = nil, showanimation = true)
    ...
    if @move == :JUDGMENT
      if attacker.itemWorks?
        m = @move.to_s
        if PBStuff::PLATEITEMS.include?(attacker.item) || (PBStuff::TYPETOZCRYSTAL.values.include?(attacker.item) && attacker.item != :NORMALIUMZ)
    ...
    if @move == :MULTIATTACK
      if attacker.itemWorks?
        m = @move.to_s
        m.concat($cache.items[attacker.item].checkFlag?(:memory).to_s).to_sym if $cache.items[attacker.item].checkFlag?(:memory)
        @battle.pbAnimation(m, attacker, opponent, hitnum)
      elsif attacker.species == :SILVALLY
        type = attacker.type1
        m = @move.to_s.concat(type.to_s).to_sym
        @battle.pbAnimation(m, attacker, opponent, hitnum)
      else
        @battle.pbAnimation(id, attacker, opponent, hitnum)
      end
...
################################################################################
class PokeBattle_Move_0E5 < PokeBattle_AllTargetMove
  def pbEffect(attacker, opponent, hitnum = 0, alltargets = nil, showanimation = true)
    failed = true
    for i in 0...4
    ...
    return -1 if priorityBlockedForAllTargetMoves?(attacker)
    ...
    for i in 0...4
      if @battle.battlers[i].effects[:PerishSong] == 0
        if @battle.battlers[i].ability == :SOUNDPROOF && !(@battle.battlers[i].moldbroken)
          @battle.pbDisplay(_INTL("{1}'s {2} blocks {3}!", @battle.battlers[i].pbThis, getAbilityName(@battle.battlers[i].ability), @name))
        elsif priorityCheck(attacker) > 0 && @battle.FE == :PSYTERRAIN && !@battle.battlers[i].isAirborne? && @battle.battlers[i].pbPartner != attacker && @battle.battlers[i] != attacker
          @battle.pbDisplay(_INTL("{1} wasn't affected!", @battle.battlers[i].pbThis))
        else
...
class PokeBattle_Move_11C < PokeBattle_Move
  def pbEffect(attacker, opponent, hitnum = 0, alltargets = nil, showanimation = true)
    ...
    if opponent.damagestate.calcdamage > 0 && !opponent.damagestate.substitute &&
      ...
      showmsg = false
      showmsg = true if opponent.hasType?(:FLYING) || opponent.ability == :LEVITATE
      if !$cache.moves[opponent.effects[:TwoTurnAttack]].nil? &&
         ($cache.moves[opponent.effects[:TwoTurnAttack]].function == 0xC9 || # Fly
          $cache.moves[opponent.effects[:TwoTurnAttack]].function == 0xCC) # Bounce
...
# Flower Shield
################################################################################
class PokeBattle_Move_150 < PokeBattle_AllTargetMove
  def pbEffect(attacker, opponent, hitnum = 0, alltargets = nil, showanimation = true)
    found = false
    for i in 0...4
    ...
    for i in 0...4
      if @battle.battlers[i].hasType?(:GRASS)
        if priorityCheck(attacker) > 0 && @battle.FE == :PSYTERRAIN && !@battle.battlers[i].isAirborne? && @battle.battlers[i].pbPartner != attacker && @battle.battlers[i] != attacker
          @battle.pbDisplay(_INTL("{1} wasn't affected!", @battle.battlers[i].pbThis))
          next
        end
  ...
  def pbEffect(attacker, opponent, hitnum = 0, alltargets = nil, showanimation = true)
    ...
    for i in 0...4
      if (@battle.battlers[i].hasType?(:GRASS) && !@battle.battlers[i].isAirborne?) &&
        (priorityCheck(attacker) <= 0 || @battle.FE != :PSYTERRAIN || @battle.battlers[i].pbPartner == attacker || @battle.battlers[i] == attacker)
        found = true
        break
      end
    end
    ...
    for i in 0...4
      if @battle.battlers[i].hasType?(:GRASS) && !@battle.battlers[i].isAirborne?
        if priorityCheck(attacker) > 0 && @battle.FE == :PSYTERRAIN && @battle.battlers[i].pbPartner != attacker && @battle.battlers[i] != attacker
          @battle.pbDisplay(_INTL("{1} wasn't affected!", @battle.battlers[i].pbThis))
          next
        end
  ...
  def pbEffect(attacker, opponent, hitnum = 0, alltargets = nil, showanimation = true)
    ...
    if opponent.ability == (:SAPSIPPER) && !(opponent.moldbroken)
      if opponent.pbCanIncreaseStatStage?(PBStats::ATTACK)
        ...
        @battle.pbCommonAnimation("StatUp", opponent, nil)
        @battle.pbDisplay(_INTL("{1}'s {2} raised its Attack!", opponent.pbThis, $cache.abil[opponent.ability].name))
      else
        @battle.pbDisplay(_INTL("{1}'s {2} made {3} ineffective!", opponent.pbThis, $cache.abil[opponent.ability].name, self.name))
      end
      return ret
    end
    pbShowAnimation(@move, attacker, opponent, hitnum, alltargets, showanimation = false)
...
class PokeBattle_Move_30A < PokeBattle_Move
  def pbOnStartUse(attacker)
    bearer = @battle.pbCheckGlobalAbility(:DAMP)
    if bearer != nil
      @battle.pbDisplay(_INTL("{1}'s {2} prevents {3} from using {4}!", bearer.pbThis, $cache.abil[bearer.ability].name, attacker.pbThis(true), @name))
      return false
    end
    @battle.pbAnimation(@move, attacker, nil)
    pbShowAnimation(@move, attacker, nil)
