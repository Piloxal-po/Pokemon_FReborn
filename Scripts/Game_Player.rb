...
class Game_Player < Game_Character
  ...
  def playBumpSE
    ...
    # check events
    # npcs
    if character.start_with?("trchar") || character.start_with?("NPC ") || character.start_with?("rby_char")
      pbSEPlay("bump-npc")
    # overworld pokemon
    elsif character.start_with?("pkmn_")
      pbSEPlay("bump-pkmn")
    # berry tree
    elsif character.start_with?("berrytree")
      pbSEPlay("bump-berrytree")
    # cut tree
    elsif eventName == "Tree"
      pbSEPlay("bump-cut")
    # headbutt tree
    elsif eventName == "HeadbuttTree"
      pbSEPlay("bump-headbutt")
    # rock smash rock
    elsif eventName == "Rock"
      pbSEPlay("bump-rocksmash")
    # breakable mirrors
    elsif eventName == "Glass"
      pbSEPlay("bump-mirror")
    # strength rocks handled elsewhere
    # zygarde cells
    elsif character == "zycell"
      pbSEPlay("bump-zcell")
    # item balls
    elsif character.start_with?("itemball")
      pbSEPlay("bump-item")
    # rock climb check names? found one in celestine that's wrong
    elsif eventName.start_with?("RockClimb") && eventHasCommands
      pbSEPlay("bump-rockclimb")

    # check terrains
    elsif pbIsJustWaterTag?(terrain)
      pbSEPlay("bump-water")
    # bad water
    elsif pbIsGrimeTag?(terrain)
      pbSEPlay("bump-grime")
    # waterfall
    elsif terrain == PBTerrain::WaterfallCrest || terrain == PBTerrain::Waterfall
      pbSEPlay("bump-waterfall")
    # ledge
    elsif terrain == PBTerrain::Ledge
      pbSEPlay("bump-ledge")
    else
      pbSEPlay("bump")
    end
